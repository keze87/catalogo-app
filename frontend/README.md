# app-catalogo-frontend

Interfaz web del catálogo de productos: una grilla de fichas con su foto, un buscador, filtros
por categoría y una ficha ampliada por producto. Es una SPA escrita en **React + Vite**.
...
Necesita **la API corriendo en `http://localhost:8000`**. Con eso:
...
`localhost:8000`; está configurado en [`vite.config.js`](vite.config.js).
...
|---|---|---|
| `API_HOST` | `catalogo-api` | Host del backend |
...
src/components/       grilla de productos, filtros, navbar y ficha ampliada
